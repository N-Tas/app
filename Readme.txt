...
- The BGManager has an instance of the SQLAlchemy interface so it can put and retrieve polynomials to and from the Database.
- The SQLAlchemy interface is raw SQL wrapper and manages the connection and statements for the Database.
...
- To ensure that there is not a linear calculation time for the polynomials entering the queue, every polynomial has a random calculation time.
...
- The C++ shared library is invoked in the worker method. 
- For the bindings, I've used ctypes.
- After the compute, the object is updated in the DB over its ID, which is passed to the queue with the model. 

3. GET{ID}
- Invokes the get method of the BGManager and passes the entered ID to it, which is invoking the get_polynomial method of the instance of the SQLAlchemy interface.
...
- The FastAPI converts the dictionary data directly into JSON.

4. GET
- Invokes the get method of the BGManager, but this time no ID is passed.
- The GET works in the same way as the GET{ID}.
- In this case the invoked method retrieves all polynomials from the Database and it returns a list of polynomial dictionaries. 
